...
def approve_leave(request, leave_id):
    ...
    return redirect('manager_dashboard' if request.user.role == 'manager' else 'superuser_dashboard')
...
def leave_summary(request, user_id):
    ...
    return render(request, 'leaves/leave_summary.html', {
        'user': user,
...
# =========================================================
# 🔥 NEW: CALENDAR EVENTS API (FIXES YOUR ERROR)
# =========================================================
from django.http import JsonResponse  # make sure this is added

@login_required
def leave_events(request):

    # 🔥 manual role check (NO redirect)
    if request.user.role != 'manager':
    ...
    for leave in leaves:
        events.append({
            ...
            "color": "green" if leave.status == "Approved"
                     else "red" if leave.status == "Rejected"
                     else "orange",
            "status": leave.status
        })
